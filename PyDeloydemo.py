...
class PyToExeConverter(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.show_cmd_btn = QPushButton('Command')
        self.show_cmd_btn.clicked.connect(self.toggle_command)
        self.show_cmd_btn.setMinimumHeight(32)
        btn_layout.addWidget(self.show_cmd_btn, 1)
    ...
    def toggle_command(self):
        """Show command in a popup dialog"""
        ...
        # Create custom dialog with QDialog
        from PyQt5.QtWidgets import QDialog, QDialogButtonBox
        
        dialog = QDialog(self)
        dialog.setWindowTitle('PyInstaller Command')
    ...
    def update_exclude_list_colors(self):
        """Mark modules that are in use with different text color"""
        ...
        for i in range(self.exclude_list.count()):
            ...
            if module_name in self.used_modules:
                # Red for modules in use
                item.setForeground(QColor(180, 0, 0))
            else:
                # Green for safe to exclude
    ...
    def update_command(self):
        pass  # No longer needed since command is shown in popup
...
def main():
    ...
    # Center window with slight upward offset
