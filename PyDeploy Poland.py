    ...
    def run(self):
        try:
            ...
            if process.returncode == 0:
                self.finished_signal.emit(True, "Budowanie zakończone pomyślnie.")
            else:
                self.finished_signal.emit(False, "Błąd podczas budowania. Sprawdź logi, aby uzyskać więcej informacji.")
...
class CollectModulesDialog(QDialog):
    def __init__(self, modules, selected_modules):
        super().__init__()
        self.setWindowTitle("Wybierz moduły do --collect-all")
        self.resize(350, 400)
...
class DropArea(QLabel):
    def __init__(self, parent=None):
        super().__init__("🗂️ Przeciągnij i upuść tutaj pliki .py, .ico oraz dodatkowe pliki")
...
class PyInstallerBuilder(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Budowniczy PyInstaller autorstwa Minh Duy")
        self.resize(750, 730)
        self.extra_files = []
    ...
    def _init_ui(self):
        ...
        self.py_path_edit.setPlaceholderText("Wybierz główny plik .py...")
        btn_py = QPushButton("Wybierz plik .py")
        ...
        self.icon_path_edit.setPlaceholderText("Wybierz ikonę (.ico) (opcjonalnie)...")
        btn_icon = QPushButton("Wybierz ikonę")
        ...
        self.extra_files_edit.setPlaceholderText("Wybrane dodatkowe pliki")
        self.extra_files_edit.setReadOnly(True)
        btn_add_files = QPushButton("Dodaj dodatkowe pliki")
        ...
        self.dist_path_edit.setPlaceholderText("Folder do zapisu pliku EXE (dist)...")
        btn_dist = QPushButton("Wybierz folder wyjściowy")
        btn_open_dist = QPushButton("Otwórz folder z EXE")
        ...
        self.chk_onefile = QCheckBox("Jeden plik (--onefile)")
        self.chk_noconsole = QCheckBox("Ukryj konsolę (--noconsole)")
        self.chk_collectall = QCheckBox("Zbierz cały moduł (--collect-all)")
        ...
        layout.addWidget(QLabel("Polecenie do uruchomienia (edytowalne):"))
        ...
        layout.addWidget(QLabel("Dziennik budowania:"))
        ...
        self.btn_build = QPushButton("Buduj")
    ...
    def select_py_file(self):
        file, _ = QFileDialog.getOpenFileName(self, "Wybierz plik .py", "", "Pliki Python (*.py)")
        if file:
            self.py_path_edit.setText(file)

    def select_icon_file(self):
        file, _ = QFileDialog.getOpenFileName(self, "Wybierz ikonę (.ico)", "", "Pliki ikon (*.ico)")
        if file:
            self.icon_path_edit.setText(file)

    def add_extra_files(self):
        files, _ = QFileDialog.getOpenFileNames(self, "Wybierz dodatkowe pliki")
        if files:
            for f in files:
    ...
    def select_dist_folder(self):
        folder = QFileDialog.getExistingDirectory(self, "Wybierz folder wyjściowy dla EXE")
        if folder:
    ...
    def on_collectall_toggled(self, state):
        if self.chk_collectall.isChecked():
            ...
            if not os.path.isfile(pyfile):
                QMessageBox.warning(self, "Błąd", "Proszę wybrać prawidłowy plik .py przed wybraniem modułów!")
    ...
    def build_exe(self):
        ...
        if not py_file or not os.path.isfile(py_file):
            QMessageBox.warning(self, "Błąd", "Proszę wybrać prawidłowy plik .py.")
            return
        cmd = self.command_preview.text()

        self.status_label.setText("Budowanie...")
    ...
    def on_build_finished(self, success, msg):
        ...
        if success:
            self.status_label.setText("Budowanie zakończone.")
            self.log_text.append("\n=== Budowanie zakończone ===")
            QMessageBox.information(self, "Powiadomienie", msg)
        else:
            self.status_label.setText("Błąd podczas budowania!")
