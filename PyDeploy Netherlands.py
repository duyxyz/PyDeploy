    ...
    def run(self):
        try:
            ...
            if process.returncode == 0:
                self.finished_signal.emit(True, "Build voltooid.")
            else:
                self.finished_signal.emit(False, "Build mislukt. Bekijk logs.")
        except Exception as e:
            self.finished_signal.emit(False, f"Fout: {e}")

class CollectModulesDialog(QDialog):
    def __init__(self, modules, selected_modules):
        super().__init__()
        self.setWindowTitle("Modules kiezen (--collect-all)")
        self.resize(350, 400)
...
class DropArea(QLabel):
    def __init__(self, parent=None):
        super().__init__("🗂️ Sleep .py, .ico en extra bestanden hierheen")
...
class PyInstallerBuilder(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("PyInstaller Builder - Minh Duy")
        self.resize(750, 730)
        self.extra_files = []
    ...
    def _init_ui(self):
        ...
        self.py_path_edit.setPlaceholderText("Kies je .py bestand...")
        btn_py = QPushButton(".py kiezen")
        ...
        self.icon_path_edit.setPlaceholderText("Kies icoon (.ico) optioneel...")
        btn_icon = QPushButton("Icoon kiezen")
        ...
        self.extra_files_edit.setPlaceholderText("Extra bestanden")
        self.extra_files_edit.setReadOnly(True)
        btn_add_files = QPushButton("Bestanden toevoegen")
        ...
        self.dist_path_edit.setPlaceholderText("Output map (dist)...")
        btn_dist = QPushButton("Kies map")
        btn_open_dist = QPushButton("Open map")
        ...
        self.chk_onefile = QCheckBox("Onefile")
        self.chk_noconsole = QCheckBox("Console verbergen")
        self.chk_collectall = QCheckBox("Alles verzamelen")
        ...
        layout.addWidget(QLabel("Commando (aanpasbaar):"))
        ...
        self.btn_build = QPushButton("Bouwen")
    ...
    def select_py_file(self):
        file, _ = QFileDialog.getOpenFileName(self, "Kies .py bestand", "", "Python bestanden (*.py)")
        if file:
            self.py_path_edit.setText(file)

    def select_icon_file(self):
        file, _ = QFileDialog.getOpenFileName(self, "Kies icoon (.ico)", "", "Icoon bestanden (*.ico)")
        if file:
            self.icon_path_edit.setText(file)

    def add_extra_files(self):
        files, _ = QFileDialog.getOpenFileNames(self, "Extra bestanden toevoegen")
        if files:
            for f in files:
    ...
    def select_dist_folder(self):
        folder = QFileDialog.getExistingDirectory(self, "Kies output map")
        if folder:
    ...
    def on_collectall_toggled(self, state):
        if self.chk_collectall.isChecked():
            ...
            if not os.path.isfile(pyfile):
                QMessageBox.warning(self, "Fout", "Selecteer eerst een geldig .py bestand!")
    ...
    def build_exe(self):
        ...
        if not py_file or not os.path.isfile(py_file):
            QMessageBox.warning(self, "Fout", "Selecteer een geldig .py bestand.")
            return
        cmd = self.command_preview.text()

        self.status_label.setText("Bouwen...")
    ...
    def on_build_finished(self, success, msg):
        ...
        if success:
            self.status_label.setText("Build voltooid.")
            self.log_text.append("\n=== Build voltooid ===")
            QMessageBox.information(self, "Info", msg)
        else:
            self.status_label.setText("Build mislukt!")
            self.log_text.append("\n=== Build mislukt ===")
            QMessageBox.warning(self, "Fout", msg)
