    ...
    def run(self):
        try:
            ...
            if process.returncode == 0:
                self.finished_signal.emit(True, "Build erfolgreich abgeschlossen.")
            else:
                self.finished_signal.emit(False, "Build-Fehler. Bitte überprüfen Sie die Protokolle für Details.")
        except Exception as e:
            self.finished_signal.emit(False, f"Fehler: {e}")

class CollectModulesDialog(QDialog):
    def __init__(self, modules, selected_modules):
        super().__init__()
        self.setWindowTitle("Module für --collect-all auswählen")
        self.resize(350, 400)
...
class DropArea(QLabel):
    def __init__(self, parent=None):
        super().__init__("🗂️ Ziehen Sie .py, .ico und zusätzliche Dateien hierher")
...
class PyInstallerBuilder(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("PyInstaller Builder von Minh Duy")
        self.resize(750, 730)
        self.extra_files = []
    ...
    def _init_ui(self):
        ...
        self.py_path_edit.setPlaceholderText("Haupt-.py-Datei auswählen...")
        btn_py = QPushButton(".py-Datei auswählen")
        ...
        self.icon_path_edit.setPlaceholderText("Icon (.ico) auswählen (optional)...")
        btn_icon = QPushButton("Icon auswählen")
        ...
        self.extra_files_edit.setPlaceholderText("Ausgewählte Zusatzdateien")
        self.extra_files_edit.setReadOnly(True)
        btn_add_files = QPushButton("Zusatzdateien hinzufügen")
        ...
        self.dist_path_edit.setPlaceholderText("Ordner für EXE-Ausgabe (dist)...")
        btn_dist = QPushButton("Ausgabeordner auswählen")
        btn_open_dist = QPushButton("EXE-Ordner öffnen")
        ...
        self.chk_onefile = QCheckBox("Einzeldatei (--onefile)")
        self.chk_noconsole = QCheckBox("Konsole ausblenden (--noconsole)")
        self.chk_collectall = QCheckBox("Modul vollständig sammeln (--collect-all)")
        ...
        layout.addWidget(QLabel("Auszuführender Befehl (bearbeitbar):"))
        ...
        layout.addWidget(QLabel("Build-Protokoll:"))
        ...
        self.btn_build = QPushButton("Build starten")
    ...
    def select_py_file(self):
        file, _ = QFileDialog.getOpenFileName(self, "Haupt-.py-Datei auswählen", "", "Python-Dateien (*.py)")
        if file:
            self.py_path_edit.setText(file)

    def select_icon_file(self):
        file, _ = QFileDialog.getOpenFileName(self, "Icon-Datei (.ico) auswählen", "", "Icon-Dateien (*.ico)")
        if file:
            self.icon_path_edit.setText(file)

    def add_extra_files(self):
        files, _ = QFileDialog.getOpenFileNames(self, "Zusatzdateien auswählen")
        if files:
            for f in files:
    ...
    def select_dist_folder(self):
        folder = QFileDialog.getExistingDirectory(self, "Ausgabeordner für EXE auswählen")
        if folder:
    ...
    def open_dist_folder(self):
        ...
        if os.path.isdir(folder):
            ...
            QMessageBox.warning(self, "Fehler", "Ordner existiert nicht.")

    def on_collectall_toggled(self, state):
        if self.chk_collectall.isChecked():
            pyfile = self.py_path_edit.text()
            if not os.path.isfile(pyfile):
                QMessageBox.warning(self, "Fehler", "Bitte wählen Sie eine gültige .py-Datei aus, bevor Sie Module auswählen!")
    ...
    def build_exe(self):
        ...
        if not py_file or not os.path.isfile(py_file):
            QMessageBox.warning(self, "Fehler", "Bitte wählen Sie eine gültige .py-Datei aus.")
            return
        cmd = self.command_preview.text()

        self.status_label.setText("Baue...")
    ...
    def on_build_finished(self, success, msg):
        ...
        if success:
            self.status_label.setText("Build abgeschlossen.")
            self.log_text.append("\n=== Build abgeschlossen ===")
            QMessageBox.information(self, "Benachrichtigung", msg)
        else:
            self.status_label.setText("Build-Fehler!")
            self.log_text.append("\n=== Build-Fehler ===")
