    ...
    def run(self):
        try:
            ...
            if process.returncode == 0:
                self.finished_signal.emit(True, "اكتملت عملية البناء بنجاح.")
            else:
                self.finished_signal.emit(False, "حدث خطأ أثناء البناء. يرجى مراجعة السجلات للمزيد من التفاصيل.")
        except Exception as e:
            self.finished_signal.emit(False, f"خطأ: {e}")

class CollectModulesDialog(QDialog):
    def __init__(self, modules, selected_modules):
        super().__init__()
        self.setWindowTitle("اختر الوحدات لـ --collect-all")
        self.resize(350, 400)
...
class DropArea(QLabel):
    def __init__(self, parent=None):
        super().__init__("🗂️ اسحب وأسقط ملفات .py و .ico والملفات الإضافية هنا")
...
class PyInstallerBuilder(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("منشئ PyInstaller من Minh Duy")
        self.resize(750, 730)
        self.extra_files = []
    ...
    def _init_ui(self):
        ...
        self.py_path_edit.setPlaceholderText("اختر ملف .py الرئيسي...")
        btn_py = QPushButton("اختر ملف .py")
        ...
        self.icon_path_edit.setPlaceholderText("اختر الأيقونة (.ico) (اختياري)...")
        btn_icon = QPushButton("اختر الأيقونة")
        ...
        self.extra_files_edit.setPlaceholderText("الملفات الإضافية المحددة")
        self.extra_files_edit.setReadOnly(True)
        btn_add_files = QPushButton("أضف ملفات إضافية")
        ...
        self.dist_path_edit.setPlaceholderText("مجلد حفظ ملف EXE (dist)...")
        btn_dist = QPushButton("اختر مجلد الإخراج")
        btn_open_dist = QPushButton("افتح مجلد EXE")
        ...
        self.chk_onefile = QCheckBox("ملف واحد (--onefile)")
        self.chk_noconsole = QCheckBox("إخفاء وحدة التحكم (--noconsole)")
        self.chk_collectall = QCheckBox("جمع جميع الوحدات (--collect-all)")
        ...
        layout.addWidget(QLabel("الأمر للتنفيذ (قابل للتعديل):"))
        ...
        layout.addWidget(QLabel("سجل البناء:"))
        ...
        self.btn_build = QPushButton("ابدأ البناء")
    ...
    def select_py_file(self):
        file, _ = QFileDialog.getOpenFileName(self, "اختر ملف .py", "", "ملفات بايثون (*.py)")
        if file:
            self.py_path_edit.setText(file)

    def select_icon_file(self):
        file, _ = QFileDialog.getOpenFileName(self, "اختر الأيقونة (.ico)", "", "ملفات الأيقونات (*.ico)")
        if file:
            self.icon_path_edit.setText(file)

    def add_extra_files(self):
        files, _ = QFileDialog.getOpenFileNames(self, "اختر ملفات إضافية")
        if files:
            for f in files:
    ...
    def select_dist_folder(self):
        folder = QFileDialog.getExistingDirectory(self, "اختر مجلد إخراج EXE")
        if folder:
    ...
    def open_dist_folder(self):
        ...
        if os.path.isdir(folder):
            ...
            QMessageBox.warning(self, "خطأ", "المجلد غير موجود.")

    def on_collectall_toggled(self, state):
        if self.chk_collectall.isChecked():
            pyfile = self.py_path_edit.text()
            if not os.path.isfile(pyfile):
                QMessageBox.warning(self, "خطأ", "يرجى اختيار ملف .py صالح قبل اختيار الوحدات!")
    ...
    def build_exe(self):
        ...
        if not py_file or not os.path.isfile(py_file):
            QMessageBox.warning(self, "خطأ", "يرجى اختيار ملف .py صالح.")
            return
        cmd = self.command_preview.text()

        self.status_label.setText("جارٍ البناء...")
    ...
    def on_build_finished(self, success, msg):
        ...
        if success:
            self.status_label.setText("اكتمل البناء.")
            self.log_text.append("\n=== اكتمل البناء ===")
            QMessageBox.information(self, "إشعار", msg)
        else:
            self.status_label.setText("خطأ في البناء!")
            self.log_text.append("\n=== خطأ في البناء ===")
