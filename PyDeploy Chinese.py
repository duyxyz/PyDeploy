    ...
    def run(self):
        try:
            ...
            if process.returncode == 0:
                self.finished_signal.emit(True, "构建成功完成。")
            else:
                self.finished_signal.emit(False, "构建出错。请查看日志以获取详细信息。")
...
class CollectModulesDialog(QDialog):
    def __init__(self, modules, selected_modules):
        super().__init__()
        self.setWindowTitle("选择 --collect-all 的模块")
        self.resize(350, 400)
...
class DropArea(QLabel):
    def __init__(self, parent=None):
        super().__init__("🗂️ 拖放 .py、.ico 及其他文件到这里")
...
class PyInstallerBuilder(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("PyInstaller 构建器 by Minh Duy")
        self.resize(750, 730)
        self.extra_files = []
    ...
    def _init_ui(self):
        ...
        self.py_path_edit.setPlaceholderText("选择主 .py 文件...")
        btn_py = QPushButton("选择 .py 文件")
        ...
        self.icon_path_edit.setPlaceholderText("选择图标 (.ico)（可选）...")
        btn_icon = QPushButton("选择图标")
        ...
        self.extra_files_edit.setPlaceholderText("已选附加文件")
        self.extra_files_edit.setReadOnly(True)
        btn_add_files = QPushButton("添加附加文件")
        ...
        self.dist_path_edit.setPlaceholderText("EXE 输出文件夹 (dist)...")
        btn_dist = QPushButton("选择输出文件夹")
        btn_open_dist = QPushButton("打开 EXE 文件夹")
        ...
        self.chk_onefile = QCheckBox("单文件 (--onefile)")
        self.chk_noconsole = QCheckBox("隐藏控制台 (--noconsole)")
        self.chk_collectall = QCheckBox("收集模块全部内容 (--collect-all)")
        ...
        layout.addWidget(QLabel("要运行的命令（可编辑）："))
    ...
    def select_py_file(self):
        file, _ = QFileDialog.getOpenFileName(self, "选择 .py 文件", "", "Python 文件 (*.py)")
        if file:
            self.py_path_edit.setText(file)

    def select_icon_file(self):
        file, _ = QFileDialog.getOpenFileName(self, "选择图标 (.ico)", "", "图标文件 (*.ico)")
        if file:
            self.icon_path_edit.setText(file)

    def add_extra_files(self):
        files, _ = QFileDialog.getOpenFileNames(self, "选择附加文件")
        if files:
            for f in files:
    ...
    def select_dist_folder(self):
        folder = QFileDialog.getExistingDirectory(self, "选择 EXE 输出文件夹")
        if folder:
    ...
    def on_collectall_toggled(self, state):
        if self.chk_collectall.isChecked():
            ...
            if not os.path.isfile(pyfile):
                QMessageBox.warning(self, "错误", "请先选择有效的 .py 文件后再选择模块！")
    ...
    def build_exe(self):
        ...
        if not py_file or not os.path.isfile(py_file):
            QMessageBox.warning(self, "错误", "请选择有效的 .py 文件。")
            return
    ...
    def on_build_finished(self, success, msg):
        ...
        if success:
            ...
            self.status_label.setText("构建错误！")
            self.log_text.append("\n=== 构建错误 ===")
