    ...
    def run(self):
        try:
            ...
            if process.returncode == 0:
                self.finished_signal.emit(True, "ビルドが正常に完了しました。")
            else:
                self.finished_signal.emit(False, "ビルド中にエラーが発生しました。詳細はログを確認してください。")
...
class CollectModulesDialog(QDialog):
    def __init__(self, modules, selected_modules):
        super().__init__()
        self.setWindowTitle(" --collect-all 用のモジュールを選択してください")
        self.resize(350, 400)
...
class DropArea(QLabel):
    def __init__(self, parent=None):
        super().__init__("🗂️ .py、.ico、および追加ファイルをここにドラッグ＆ドロップしてください")
...
class PyInstallerBuilder(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Minh Duy の PyInstaller ビルダー")
        self.resize(750, 730)
        self.extra_files = []
    ...
    def _init_ui(self):
        ...
        self.py_path_edit.setPlaceholderText("メインの .py ファイルを選択してください...")
        btn_py = QPushButton(".py ファイルを選択")
        ...
        self.icon_path_edit.setPlaceholderText("アイコン (.ico) を選択（任意）...")
        btn_icon = QPushButton("アイコンを選択")
        ...
        self.extra_files_edit.setPlaceholderText("選択した追加ファイル")
        self.extra_files_edit.setReadOnly(True)
        btn_add_files = QPushButton("追加ファイルを追加")
        ...
        self.dist_path_edit.setPlaceholderText("EXE を保存するフォルダ (dist)...")
        btn_dist = QPushButton("出力フォルダを選択")
        btn_open_dist = QPushButton("EXE フォルダを開く")
        ...
        self.chk_onefile = QCheckBox("ワンファイルバンドル (--onefile)")
        self.chk_noconsole = QCheckBox("コンソールを隠す (--noconsole)")
        self.chk_collectall = QCheckBox("モジュール全収集 (--collect-all)")
        ...
        layout.addWidget(QLabel("実行コマンド（編集可能）："))
        ...
        self.btn_build = QPushButton("ビルド")
    ...
    def select_py_file(self):
        file, _ = QFileDialog.getOpenFileName(self, ".py ファイルを選択", "", "Python ファイル (*.py)")
        if file:
            self.py_path_edit.setText(file)

    def select_icon_file(self):
        file, _ = QFileDialog.getOpenFileName(self, "アイコン (.ico) を選択", "", "アイコンファイル (*.ico)")
        if file:
            self.icon_path_edit.setText(file)

    def add_extra_files(self):
        files, _ = QFileDialog.getOpenFileNames(self, "追加ファイルを選択")
        if files:
            for f in files:
    ...
    def select_dist_folder(self):
        folder = QFileDialog.getExistingDirectory(self, "EXE 出力フォルダを選択")
        if folder:
    ...
    def on_collectall_toggled(self, state):
        if self.chk_collectall.isChecked():
            ...
            if not os.path.isfile(pyfile):
                QMessageBox.warning(self, "エラー", "有効な .py ファイルを選択してください。")
    ...
    def build_exe(self):
        ...
        self.status_label.setText("ビルド中...")
    ...
    def on_build_finished(self, success, msg):
        ...
        if success:
            self.status_label.setText("ビルド完了。")
            self.log_text.append("\n=== ビルド完了 ===")
            QMessageBox.information(self, "通知", msg)
        else:
            self.status_label.setText("ビルドエラー！")
            self.log_text.append("\n=== ビルドエラー ===")
