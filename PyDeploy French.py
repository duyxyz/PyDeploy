    ...
    def run(self):
        try:
            ...
            if process.returncode == 0:
                self.finished_signal.emit(True, "Compilation terminée avec succès.")
            else:
                self.finished_signal.emit(False, "Erreur de compilation. Vérifiez les logs pour plus de détails.")
...
class CollectModulesDialog(QDialog):
    def __init__(self, modules, selected_modules):
        super().__init__()
        self.setWindowTitle("Sélectionnez les modules pour --collect-all")
        self.resize(350, 400)
...
class DropArea(QLabel):
    def __init__(self, parent=None):
        super().__init__("🗂️ Glissez et déposez ici les fichiers .py, .ico et autres fichiers supplémentaires")
...
class PyInstallerBuilder(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Constructeur PyInstaller par Minh Duy")
        self.resize(750, 730)
        self.extra_files = []
    ...
    def _init_ui(self):
        ...
        self.py_path_edit.setPlaceholderText("Sélectionnez le fichier .py principal...")
        btn_py = QPushButton("Sélectionner le fichier .py")
        ...
        self.icon_path_edit.setPlaceholderText("Sélectionnez l'icône (.ico) (optionnel)...")
        btn_icon = QPushButton("Sélectionner l'icône")
        ...
        self.extra_files_edit.setPlaceholderText("Fichiers supplémentaires sélectionnés")
        self.extra_files_edit.setReadOnly(True)
        btn_add_files = QPushButton("Ajouter des fichiers supplémentaires")
        ...
        self.dist_path_edit.setPlaceholderText("Dossier de sortie EXE (dist)...")
        btn_dist = QPushButton("Sélectionner le dossier de sortie")
        btn_open_dist = QPushButton("Ouvrir le dossier EXE")
        ...
        self.chk_onefile = QCheckBox("Un seul fichier (--onefile)")
        self.chk_noconsole = QCheckBox("Cacher la console (--noconsole)")
        self.chk_collectall = QCheckBox("Collecter tous les modules (--collect-all)")
        ...
        layout.addWidget(QLabel("Commande à exécuter (modifiable) :"))
        ...
        layout.addWidget(QLabel("Journal de construction :"))
        ...
        self.btn_build = QPushButton("Construire")
    ...
    def select_py_file(self):
        file, _ = QFileDialog.getOpenFileName(self, "Sélectionnez le fichier .py", "", "Fichiers Python (*.py)")
        if file:
            self.py_path_edit.setText(file)

    def select_icon_file(self):
        file, _ = QFileDialog.getOpenFileName(self, "Sélectionnez l'icône (.ico)", "", "Fichiers icônes (*.ico)")
        if file:
            self.icon_path_edit.setText(file)

    def add_extra_files(self):
        files, _ = QFileDialog.getOpenFileNames(self, "Sélectionnez les fichiers supplémentaires")
        if files:
            for f in files:
    ...
    def select_dist_folder(self):
        folder = QFileDialog.getExistingDirectory(self, "Sélectionnez le dossier de sortie EXE")
        if folder:
    ...
    def on_collectall_toggled(self, state):
        if self.chk_collectall.isChecked():
            ...
            if not os.path.isfile(pyfile):
                QMessageBox.warning(self, "Erreur", "Veuillez sélectionner un fichier .py valide avant de sélectionner les modules !")
    ...
    def build_exe(self):
        ...
        if not py_file or not os.path.isfile(py_file):
            QMessageBox.warning(self, "Erreur", "Veuillez sélectionner un fichier .py valide.")
            return
    ...
    def on_build_finished(self, success, msg):
        ...
        if success:
            self.status_label.setText("Compilation terminée.")
            self.log_text.append("\n=== Compilation terminée ===")
            QMessageBox.information(self, "Notification", msg)
        else:
            self.status_label.setText("Erreur de compilation !")
